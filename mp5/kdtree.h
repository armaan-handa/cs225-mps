...
class KDTree
{
  ...
  public:
    ...
    Point<Dim> findNearestNeighbor(const Point<Dim>& query, KDTreeNode * subRoot, int dim) const;
  ...
  private:
    ...
    int partition(int left, int right, int middle, int dim);
    Point<Dim> quickSelect(int left, int right, int middle, int dim);
    KDTreeNode* constructor(int left, int right, int dim);
