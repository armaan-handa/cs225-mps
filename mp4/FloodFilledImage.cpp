...
Animation FloodFilledImage::animate(unsigned frameInterval) {
  ...
  /** @todo [Part 2] */
  unsigned count = 0;
  unsigned size = traversals.size();
  animation.addFrame(*myPNG);
  for (unsigned i = 0; i < size; i++){
    for(ImageTraversal::Iterator it = traversals.front()->begin(); it != traversals.front()->end(); ++it){
      unsigned x = it.current.x;
      unsigned y = it.current.y;
      myPNG->getPixel(x, y) = colorpickers.front()->getColor(x, y);
